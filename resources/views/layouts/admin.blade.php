...
<footer class="footer bg-dark text-white d-flex justify-content-between align-items-center px-3  mt-4 m-auto" style="position: relative; bottom: 0; width: 97.5%;">
    <div class="footer-left d-flex align-items-center">
        @if($setting && $setting->logo_path)
            <a href="{{ $setting->company_url ?? '#' }}">
                <img src="{{ asset($setting->logo_path) }}" alt="Logo" height="30" width="80" class="me-2">
            </a>
        @endif
        <span>&copy; {{ date('Y') }}</span>
    </div>
    <div class="footer-right">
        <a href="{{ $setting->company_url ?? '#' }}" class="text-white text-decoration-none">
            <strong>{{ $setting->company_name ?? 'Big Nation 7 Infotech' }}</strong>
        </a>
    </div>
</footer>
